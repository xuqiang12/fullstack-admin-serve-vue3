<template>
  ...
  >
    <!-- :row-key="getRowKey"  :cell-class-name="addTdClass" -->
    <template v-for="(item, index) of afterMergerColumns" :key="index">
      <el-table-column min-width="100" :width="item.width ? item.width : 'auto'" :label="item.label" :align="item.align ? item.align : 'center'" :prop="item.img ? 'img' : ''" :fixed="item.fixed">
                ...
                <slot :row="row" :index="$index" :name="btn.btnlable">
                  ...
                  <div v-if="btnVisible(row, btn)">
                    <div v-if="btn.btnType === 'svgIcon' || btn.btnType === 'icon'">
                      ...
                      <el-link v-else :underline="false" :disabled="evals(row, btn.disabled)" :style="evals(row, btn.disabled) ? '' : 'color:#1890ff'" @click.stop="handleButton(btn, row, $index)">
                    ...
                    <el-link v-else :underline="false" type="primary" :disabled="evals(row, btn.disabled)" :style="evals(row, item.style)" @click.stop="handleButton(btn, row, $index)">
                      {{ btn.btnlable }}
                    </el-link>
  ...
  originalBtn.forEach(element => {
    if (Array.isArray(element.pram)) {
      if (element.role) {
        ...
        const visible = element.pram.filter((item: any) => (item.role && receiveBtn.some((routebtn: any) => routebtn.type === item.flag)) || !item.role);
        if (visible.length > 0) rbacBtn.push({ ...element, pram: visible });
      } else {
